...
def finish_heating(G, current_node, subtask, env, agent, target_obj_instance):
    actions = []; total_time = 0
    target_obj = subtask['object_target']
...
def start_cooling(G, current_node, subtask, env, agent):
    ...
    if "Nothing happens" in agent.feedback:
        return 0,0,0,0
    total_time += 1
    target_obj_instance = [target_obj_instance, env.last_action['objectId']]
    ...
    total_time += 1
    actions.append("put " + target_obj_instance[0] + " in fridge 1")
    agent.step(("put " + target_obj_instance[0] + " in fridge 1"))
    print(agent.feedback)

    total_time += 1
    actions.append("close fridge 1")
    agent.step("close fridge 1")
    print(agent.feedback)

    return actions, total_time, 'fridge 1', env, target_obj_instance


def finish_cooling(G, current_node, subtask, env, agent, target_obj_instance):
    actions = []; total_time = 0
    target_obj = subtask['object_target']
    parent_obj = subtask['parent_target']

    if current_node != 'fridge 1':
        next_node, time_taken = get_shortest_path_distance(G, current_node, 'fridge 1')
        if next_node:
            total_time += time_taken
        else:
    ...
    inside_fridge = env.object_states[target_obj_instance[1]]['inside_fridge']
    if inside_fridge < TIME_BOUNDARIES[subtask['desired_state']][0]:
        agent.step("wait for " + str(TIME_BOUNDARIES[subtask['desired_state']][0]-inside_fridge) + " time steps")
        print(agent.feedback)
    elif inside_fridge > TIME_BOUNDARIES[subtask['desired_state']][1]:
        return 0,0,0,0
    ...
    actions.append("take " + target_obj_instance[0] + " from fridge 1")
    agent.step("take " + target_obj_instance[0] + " from fridge 1")
    print(agent.feedback)
    total_time += 1
    # for visible_object in G.nodes[current_node]['visible_objects']:
    #     if visible_object.split('|')[0] == target_obj:
    ...
    actions.append("put " + target_obj_instance + " on/in " + current_node)
    agent.step("put " + target_obj_instance + " on/in " + current_node)
    print(agent.feedback)
    total_time += 1

    return actions, total_time, current_node, env



def start_cleaning(G, current_node, subtask, env, agent):
    actions = []; total_time = 0
    target_obj = subtask['object_target']
    next_node, time_taken = get_shortest_path_distance(G, current_node, target_obj)
    if next_node:
    ...
    return actions, total_time, 'sinkbasin 1', env, target_obj_instance


def finish_cleaning(G, current_node, subtask, env, agent, target_obj_instance):
    actions = []; total_time = 0
    target_obj = subtask['object_target']
    parent_obj = subtask['parent_target']
...
def execute_subtask(G, current_node, subtask, env, agent, target_obj_instances):
    if subtask['stage'] == 'start':
        if subtask['task_type'] == "pick_heat_then_place_in_recep":      actions, time_taken, updated_node, env, target_obj_instance = start_heating(G, current_node, subtask, env, agent)
        elif subtask['task_type'] == "pick_cool_then_place_in_recep":    actions, time_taken, updated_node, env, target_obj_instance = start_cooling(G, current_node, subtask, env, agent)
        elif subtask['task_type'] == "pick_clean_then_place_in_recep":   actions, time_taken, updated_node, env, target_obj_instance = start_cleaning(G, current_node, subtask, env, agent)
        target_obj_instances[subtask['task_id']] = target_obj_instance
    else:
        if subtask['task_type'] == "pick_heat_then_place_in_recep":      actions, time_taken, updated_node, env = finish_heating(G, current_node, subtask, env, agent, target_obj_instances[subtask['task_id']])
        elif subtask['task_type'] == "pick_cool_then_place_in_recep":    actions, time_taken, updated_node, env = finish_cooling(G, current_node, subtask, env, agent, target_obj_instances[subtask['task_id']])
        elif subtask['task_type'] == "pick_clean_then_place_in_recep":   actions, time_taken, updated_node, env = finish_cleaning(G, current_node, subtask, env, agent, target_obj_instances[subtask['task_id']])
    return actions, time_taken, updated_node, env, target_obj_instances
...
def calculate_consumed_time(G, subtasks, env, agent):
    ...
    current_node = 'spawned_location'; target_obj_instances = {}
    for subtask in subtasks:
        actions, time_taken, updated_node, env, target_obj_instances = execute_subtask(G, current_node, subtask, env, agent, target_obj_instances)
