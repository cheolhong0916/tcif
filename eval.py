...
def main(args):
    ...
    for i in trange(len(problems)):
        ...
        cmd_log[root] = []
        while env.steps_taken < 2500:
            # get action cmd from input
            cmd = input('> ')

            # get action cmd from the model
            ########## Edit This Part ##########
            # cmd = 
            ####################################

            cmd_log[root].append([env.steps_taken, cmd])

            agent.step(cmd)
            print(agent.feedback)
            done, results = env.get_goal_satisfied()
            if done:
                print("You won!")
                cmd_log['success'] = 1
                cmd_log['goal_condition'] = results
                break
        ...
        with open(os.path.join(ALFWORLD_DATA, "results", args.split, args.method_name + '_' + args.start_idx + '_' + args.end_idx + '.json'), 'w') as f:
            json.dump(cmd_log,f,indent=4)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--method_name', type=str, default='default')
    args = parser.parse_args()

    if args.problem is None:
        ### json_3.1.0
        json_path = os.path.join(ALFWORLD_DATA, "json_3.1.0/"+args.split)
        problems = [os.path.join(json_path, item) for item in os.listdir(json_path)]
        problems = sorted(problems)
        try: args.problem = problems[int(args.start_idx):int(args.end_idx)]
        except: args.problem = problems
