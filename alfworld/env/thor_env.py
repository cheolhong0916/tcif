...
class ThorEnv(Controller):
    ...
    def __init__(self, x_display=constants.X_DISPLAY,
        ...
        self.COOL_objects = set()
        self.COLD_objects = set()
        self.FREEZING_objects = set()
        self.WARM_objects = set()
        self.HOT_objects = set()
        self.BOILING_objects = set()
        self.RINSED_objects = set()
        self.WASHED_objects = set()

        self.object_states = {}  # To track object states and time steps

        self.desired_states = {"cool":None, "heat": None}
    ...
    def set_task(self, traj, args, reward_type='sparse', max_episode_length=2000):
        ...
        self.task = get_task(traj, self, args, reward_type=reward_type, max_episode_length=max_episode_length)


        # task_type = traj['task_type']
        # self.task = get_task(task_type, traj, self, args, reward_type=reward_type, max_episode_length=max_episode_length)

        ## temp 대신 traj_data에 desired_state 정보 추가해서 불러오도록 구현해야 함.

        # temp

        # if 'cool' in task_type:
        #     self.desired_states['cool'] = random.choice(["COOL", "COLD", "FREEZING"])
        #     traj['desired_states'] = self.desired_states
        # if 'heat' in task_type:
        #     self.desired_states['heat'] = random.choice(["WARM", "HOT", "BOILING"])
        #     traj['desired_states'] = self.desired_states
        # traj['desired_states']

        # # set desired states
        # self.desired_states = traj['desired_states']
    ...
    def update_states(self, time):
        ...
        if event.metadata['lastActionSuccess']:
            ...
            # if action['action'] == 'ToggleObjectOn' and "Microwave" in action['objectId']:
            microwave = get_objects_of_type('Microwave', event.metadata)[0]
            if microwave['isToggled']:
                heated_object_ids = microwave['receptacleObjectIds']
                if heated_object_ids is not None:
                    for obj_id in heated_object_ids:
                        if obj_id not in self.object_states:
                            self.object_states[obj_id] = {'heated_time': 0}
                        else:
                            if 'heated_time' not in self.object_states[obj_id]:
                                self.object_states[obj_id]['heated_time'] = 0
                            else:
                                self.object_states[obj_id]['heated_time'] += time
            ...
            if not fridge['isOpen']:
                if cooled_object_ids is not None:
                    for obj_id in cooled_object_ids:
                        if obj_id not in self.object_states:
                            self.object_states[obj_id] = {'cooled_time': 0}
                        else:
                            if 'cooled_time' not in self.object_states[obj_id]:
                                self.object_states[obj_id]['cooled_time'] = 0
                            else:
                                self.object_states[obj_id]['cooled_time'] += time
                        
            # clean
            # if action['action'] == 'ToggleObjectOn' and "Faucet" in action['objectId']:
            faucet = get_objects_of_type('Faucet', event.metadata)[0]
            if faucet['isToggled']:
                sinkbasin = get_objects_of_type('SinkBasin', event.metadata)[0]
                cleaned_object_ids = sinkbasin['receptacleObjectIds']
                if cleaned_object_ids is not None:
                    for obj_id in cleaned_object_ids:
                        if obj_id not in self.object_states:
                            self.object_states[obj_id] = {'cleaned_time': 0}
                        else:
                            if 'cleaned_time' not in self.object_states[obj_id]:
                                self.object_states[obj_id]['cleaned_time'] = 0
                            else:
                                self.object_states[obj_id]['cleaned_time'] += time 
                # self.cleaned_objects = self.cleaned_objects | set(cleaned_object_ids) if cleaned_object_ids is not None else set()
    ...
    def update_object_states(self):
        ...
        for obj_id, state_info in self.object_states.items():
            object_name = obj_id.split('|')[0]
            # Check for cooling state
            if 'cooled_time' in state_info:
                if state_info['cooled_time'] >= constants.TIME_THRESHOLDS["COOL"] and state_info['cooled_time'] < constants.TIME_THRESHOLDS["COLD"]:
                    self.COOL_objects.add(obj_id)
                if state_info['cooled_time'] >= constants.TIME_THRESHOLDS["COLD"] and state_info['cooled_time'] < constants.TIME_THRESHOLDS["FREEZING"]:
                    self.COLD_objects.add(obj_id)
                if state_info['cooled_time'] >= constants.TIME_THRESHOLDS["FREEZING"]:
                    self.FREEZING_objects.add(obj_id)                                    

            # Check for heating state
            if 'heated_time' in state_info:
                if state_info['heated_time'] >= constants.TIME_THRESHOLDS["WARM"] and state_info['heated_time'] < constants.TIME_THRESHOLDS["HOT"]:
                    self.WARM_objects.add(obj_id)
                if state_info['heated_time'] >= constants.TIME_THRESHOLDS["HOT"] and state_info['heated_time'] < constants.TIME_THRESHOLDS["BOILING"]:
                    self.HOT_objects.add(obj_id)
                if state_info['heated_time'] >= constants.TIME_THRESHOLDS["BOILING"]:
                    self.BOILING_objects.add(obj_id)

            # Check for cleaning state
            if 'cleaned_time' in state_info:
                if state_info['cleaned_time'] >= constants.TIME_THRESHOLDS["RINSED"]:
                    self.RINSED_objects.add(obj_id)
                if state_info['cleaned_time'] >= constants.TIME_THRESHOLDS["WASHED"]:
                    self.WASHED_objects.add(obj_id)
##############################################################################################################################################
    ...
    def get_goal_conditions_met(self):
        if self.task is None:
            ...
            return self.task.goal_conditions_met(self.last_event, self.tasks)
