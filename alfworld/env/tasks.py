...
class BaseTask(object):
    ...
    def get_targets(self):
        ...
        # slice exception
        if 'object_sliced' in self.traj[task]['pddl_params'] and self.traj[task]['pddl_params']['object_sliced']:
            targets['object'] += 'Sliced'  # Change, e.g., "Apple" -> "AppleSliced" as pickup target.

        return targets


class NewBaseTask(object):
    '''
    base class for tasks
    '''
    ...
    def __init__(self, traj, env, args, reward_type='sparse', max_episode_length=2000):
        ...
        self.args = args
        # self.task_type = self.traj['task_type']
        self.task_types = list(traj.keys())
        self.max_episode_length = max_episode_length
        self.reward_type = reward_type
        self.step_num = 0
        self.goal_finished = False
        ...
        for task_type in self.task_types:
            self.num_subgoals += len(traj[task_type]['plan']['high_pddl']) - 1 # ignore end noop
    ...
    def load_nav_graph(self):
        ...
        build navigation grid graph
        '''
        floor_plan = self.traj[self.task_types[0]]['scene']['floor_plan']
        scene_num = self.traj[self.task_types[0]]['scene']['scene_num']
        self.gt_graph = graph_obj.Graph(use_gt=True, construct_graph=True, scene_id=scene_num)
    ...
    def transition_reward(self, state):
        ...
        # get subgoal and action
        expert_plan = self.traj[self.task_types[0]]['plan']['high_pddl']
        action_type = expert_plan[self.goal_idx]['planner_action']['action']
        ...
        # step penalty
        if self.step_num > len(self.traj[self.task_types[0]]['plan']['low_actions']):
            reward += self.reward_config['Generic']['step_penalty']
    ...
    def get_target(self, var, td):
        '''
        returns the object type of a task param
        '''
        return td['pddl_params'][var] if td['pddl_params'][var] is not None else None

    def get_targets(self, td):
        '''
        returns a dictionary of all targets for the task
        '''
        targets = {
            'object': self.get_target('object_target', td),
            'parent': self.get_target('parent_target', td),
            'toggle': self.get_target('toggle_target', td),
            'mrecep': self.get_target('mrecep_target', td),
        }

        # slice exception
        if 'object_sliced' in td['pddl_params'] and td['pddl_params']['object_sliced']:
            targets['object'] += 'Sliced'  # Change, e.g., "Apple" -> "AppleSliced" as pickup target.
...
def get_task(traj, env, args, reward_type='sparse', max_episode_length=2000):
    task = globals()['CombinatedTask']
    return task(traj, env, args, reward_type=reward_type, max_episode_length=max_episode_length)
...
#     def goal_conditions_met(self, state, tasks):
#         # tasks = ['pick_cool_then_place_in_recep', 'pick_two_obj_and_place']
#         def pick_cool_then_place_in_recep():
#             '''
#             pick_cool_then_place_in_recep task
#             '''
#             ts = 3
...
class CombinatedTask(NewBaseTask):
    ...
    def pick_and_place_simple(self, state, td):
        ts = 1
        s = 0

        targets = self.get_targets(td)
        receptacles = get_objects_with_name_and_prop(targets['parent'], 'receptacle', state.metadata)
        pickupables = get_objects_with_name_and_prop(targets['object'], 'pickupable', state.metadata)
    ...
    def pick_two_obj_and_place(self, state, td):
        ts = 2
        s = 0
    ...
    def look_at_obj_in_light(self, state, td):
        ts = 2
        s = 0
    ...
    def pick_heat_then_place_in_recep(self, state, td):
        ts = 3
        s = 0
        ...
        if td['desired_state']['heat'] == 'warm':
            objs_heated = [p['objectId'] for p in pickupables if p['objectId'] in self.env.WARM_objects]
        elif td['desired_state']['heat'] == 'hot':
            objs_heated = [p['objectId'] for p in pickupables if p['objectId'] in self.env.HOT_objects]
        elif td['desired_state']['heat'] == 'boiling':
            objs_heated = [p['objectId'] for p in pickupables if p['objectId'] in self.env.BOILING_objects]
    ...
    def pick_cool_then_place_in_recep(self, state, td):
        ts = 3
        s = 0
        ...
        if td['desired_state']['cool'] == 'cool':
            objs_cooled = [p['objectId'] for p in pickupables if p['objectId'] in self.env.COOL_objects]
        elif td['desired_state']['cool'] == 'cold':
            objs_cooled = [p['objectId'] for p in pickupables if p['objectId'] in self.env.COLD_objects]
        elif td['desired_state']['cool'] == 'freezing':
            objs_cooled = [p['objectId'] for p in pickupables if p['objectId'] in self.env.FREEZING_objects]
    ...
    def pick_clean_then_place_in_recep(self, state, td):
        ts = 3
        s = 0
        ...
        if td['desired_state']['clean'] == 'rinsed':
            objs_cleaned = [p['objectId'] for p in pickupables if p['objectId'] in self.env.RINSED_objects]
        elif td['desired_state']['clean'] == 'washed':
            objs_cleaned = [p['objectId'] for p in pickupables if p['objectId'] in self.env.WASHED_objects]
